...
atrain = pd.read_csv('../data/dup/train_d.csv')
atrain.fillna(0,inplace=True)
trainy = atrain['label']
atrain.drop('label',axis=1,inplace=True)

test = pd.read_csv('../data/dup/test_d.csv')
test.fillna(-10,inplace=True)
test.drop('label',axis=1,inplace=True)

avalid = pd.read_csv('../data/dup/valid_d.csv')
...
def XGBoost_(train=atrain,y=trainy,test=test,valid=avalid,validy=validy,k=0,num_round=50,
			 gamma=.1,min_child_weight=5,max_depth=9,lamda=29,scale_pos_weight=40,
			 subsamp=0.786,col_bytree=0.876,col_bylevel=0.80,eta=0.184,file="aac"):
